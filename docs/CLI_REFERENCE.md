...
| `nxr cache clear` | Remove discovery, capability, workspace CAS, prepared-plan, store-exe, action-digest, and merkle index entries |
| `nxr cache status` | Show cache paths/sizes (`--json`: nested `{discovery,capabilities,workspace,plans}`) |
