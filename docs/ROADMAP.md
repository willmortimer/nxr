...
| **V3.1** | Workspace actions + process MVP | Local CAS, resources, `up`/`status`/`logs`/`down`, inventory, history, coalesced discovery — `v3.1.2` (packaging + release-smoke fixes; `v3.1.0`/`v3.1.1` release publish blocked). Docs drift check (`checks.*.cli-ref`) lands in **3.1.3**. |
